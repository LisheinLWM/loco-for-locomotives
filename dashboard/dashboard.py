...
def plot_average_delays_by_company(data_df: pd.DataFrame) -> None:
    ...
    # Group the data by company_name and calculate the average delay
    average_delays = data_df.groupby('company_name')['arrival_lateness'].mean().reset_index()

    plt.figure(figsize=(15, 6))
    ax = sns.barplot(x='company_name', y='arrival_lateness', data=average_delays)
...
def plot_average_delays_by_station(data_df: pd.DataFrame, station) -> None:
    ...
    average_delays = data_df.groupby('origin_station_name')['arrival_lateness'].mean().reset_index()

     # Sort the data by average delay in descending order and select the top 20 stations
    average_delays = average_delays.sort_values(by='arrival_lateness', ascending=False).head(20)

    plt.figure(figsize=(15, 6))
    ax = sns.barplot(x='origin_station_name', y='arrival_lateness', data=average_delays)
...
if __name__ == "__main__":
    st.set_page_config(page_title="Train Services Monitoring Dashboard", layout="wide")
    load_dotenv()
