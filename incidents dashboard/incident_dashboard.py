...
def display_headline_figures(incident_df: DataFrame):
    ...
    with col3:
        ...
        st.metric("OPERATOR W/ MOST INCIDENTS ",
                  f"{operator_with_highest_incidents} ({highest_incident_count})")
    with col4:
        ...
        st.metric("MOST COMMON INCIDENT PRIORITY",
                  f"{priority_with_highest_count} ({highest_incident_priority_count})")
...
def show_metrics_for_given_operator(sns_client: ServiceResource, operator_list: list[str],
    ...
    code = st.selectbox(
        'SELECT OPERATOR TO VIEW METRICS FOR', options=operator_list)
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("TOTAL SUBSCRIPTIONS, ALL OPERATORS",
                  calculate_total_subscriptions(sns_client, operator_list))
    with col2:
        st.metric(f"{code} SUBSCRIBER COUNT",
                  get_subscription_count(sns_client, code))
    idx = incident_df.groupby('incident_num')['incident_version'].idxmax()
    incident_df = incident_df.loc[idx]
    with col3:
        st.metric(f"{code} TOTAL INCIDENTS", len(
            incident_df[incident_df["operator_code"] == code]))
    with col4:
        current_time = datetime.now()
...
def bar_graph_avg_incidents_per_day_per_operator(df: DataFrame) -> None:
    ...
    operator_avg_incidents = df.groupby(['operator_code', df['start_time'].dt.date])[
        'incident_id'].count().reset_index()
    operator_avg_incidents = operator_avg_incidents.rename(
        columns={'incident_id': 'avg_incidents'})
...
def bar_graph_avg_incidents_per_day_per_route(df: DataFrame) -> None:
    ...
    route_avg_incidents = df.groupby(['route_name', df['start_time'].dt.date])[
        'incident_id'].count().reset_index()
    route_avg_incidents = route_avg_incidents.rename(
        columns={'incident_id': 'avg_incidents'})
...
if __name__ == "__main__":
    ...
    selected_operators = st.sidebar.multiselect(
        "Operator", set(incident_df["operator_name"].unique().tolist()))
    operator_filtered_df = incident_df[incident_df['operator_name'].isin(
        selected_operators)]
    ...
    selected_routes = st.sidebar.multiselect(
        "Route", set(incident_df["route_name"].unique().tolist()))
    route_filtered_df = incident_df[incident_df['route_name'].isin(
        selected_routes)]
